...
import {
  ...
  CarFront, IdCard, IndianRupee, MessageSquare, LogIn, LogOut, ClipboardList
} from "lucide-react";
import "./styles.css";
...
const MOCK_LOGIN = { mobile:"9876543210", otp:"123456" };
...
function Header({onMenu, onHome, onDashboard, onDemo, onNotifications, loggedIn, onLogin, onLogout}) {
...
function LoginPage({onLogin}) {
  ...
  const verify=()=>{
    if(otp===MOCK_LOGIN.otp){onLogin();return}
  ...
  return <main className="login-page">
    <div className="login-shell">
      <div className="login-brand">
        ...
        <div><b>Parivahan</b><span>Ministry of Road Transport & Highways</span></div>
      </div>
      <div className="login-card">
        <span className="section-kicker">Secure access</span>
        <h1>Log in to My Parivahan</h1>
        <p className="login-subtitle">Use your mobile number to access applications, vehicles and saved services.</p>
        {!sent ? <>
          <label className="login-label">Mobile number</label>
          <div className="login-input"><span>+91</span><input value={mobile} onChange={e=>setMobile(e.target.value.replace(/\D/g,"").slice(0,10))} placeholder="10-digit mobile number" inputMode="numeric"/></div>
          <button className="primary login-submit" onClick={sendOtp}>Send OTP <ArrowRight size={17}/></button>
        </> : <>
          <div className="otp-sent">OTP sent to +91 {mobile.slice(0,2)}••••••{mobile.slice(-2)}</div>
          <label className="login-label">Enter OTP</label>
          <input className="otp-input" value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="6-digit OTP" inputMode="numeric" autoFocus/>
          <button className="primary login-submit" onClick={verify}>Verify & continue <ArrowRight size={17}/></button>
          <button className="text-btn login-back" onClick={()=>{setSent(false);setOtp("");setError("")}}>Use a different number</button>
        </>}
        {error&&<div className="login-error"><AlertCircle size={16}/><span>{error}</span></div>}
        <div className="demo-credentials"><Sparkles size={15}/><div><b>Hackathon demo login</b><p>Mobile: <strong>9876543210</strong> · OTP: <strong>123456</strong></p><small>No real SMS is sent and no personal data is stored.</small></div></div>
      </div>
      <div className="login-footer"><ShieldCheck size={14}/> Mock authentication for demonstration only</div>
    </div>
  </main>
}
...
function App(){
  ...
  const go=(p)=>{
    setMenu(false);
    const protectedPages=["dashboard","renew","application","status","challan"];
    if(protectedPages.includes(p) && !loggedIn){ setPage("login"); window.scrollTo({top:0,behavior:"smooth"}); return; }
  ...
  if(page==="login") return <LoginPage onLogin={()=>{setLoggedIn(true);setPage("dashboard");window.scrollTo({top:0,behavior:"smooth"})}}/>;

  return <><Header onMenu={()=>setMenu(!menu)} onHome={go} onDashboard={()=>go("dashboard")} onDemo={()=>setDemo(true)} onNotifications={()=>setNotifications(true)} loggedIn={loggedIn} onLogin={()=>setPage("login")} onLogout={()=>{setLoggedIn(false);setPage("home")}}/>{menu&&<div className="mobile-nav"><button onClick={()=>go("home")}>Services</button><button onClick={()=>go("dashboard")}>My Parivahan</button><button onClick={()=>go("rto")}>Find an RTO</button><button onClick={()=>go("help")}>Help</button></div>}
